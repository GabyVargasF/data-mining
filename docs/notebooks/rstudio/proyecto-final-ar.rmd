...
Escribe la fecha actual (**NOTE-01**): <04 Junio 2020>
...
```{r}
NOMBRE_COMPLETO = "Gabriela Mariel Vargas Franco"
DIGITOS_MATRICULA = 01745775
```
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**): Nos dice que existen ocasiones en las que únicamente se hace la compra de pañales. (No se hizo la compra de otro item.. NO hubo co-ocurrencia)
- Calcula el valor de la métrica support para la regla `{beer} => {nuts}` (**NOTE-20**): 0.2
- Calcula el valor de la métrica confidence para la regla `{beer} => {nuts}` (**NOTE-21**): 5/15---> 1/3
- ¿Por qué no aparece listada la regla `{beer} => {nuts}`? (**NOTE-22**): Las métricas de confidence y support nos indican una exclusión de la regla.
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**): Representa el id de la sesión que cada usuario tiene en Spotify. Permite darle al usuario una mejor experiencia ya que le hace recomendaciones y listas personalizadas de acuerdo a sus interacciones frecuentes.
- ¿Cuál es el propósito de `track id`? (**NOTE-24**): Es el identificador de cada canción y nos ayuda a saber estadistísticas para hacer recomendaciones al usuario.
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**): Si, porque nos muestran co-ocurrencia.. lo que nos permite asociarlos a otros elementos del dataset (realizar relación entre las canciones dependiendo de los gustos, pausas, saltos de canciones, entre otros.)
...
- ¿Cuál es el ID y frecuencia absoluta de la sesión más frecuente? (**NOTE-26**): t_bacf06d3-9185-4183-84ea-ff0db51475ce con una frecuencia de 890
- ¿Cuántos items tiene el itemset con mayor número de ocurrencias en la lista element length distribution? (**NOTE-27**): Itemset : 20 con una ocurrencia de 1554
- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética? (**NOTE-28**): Los usuarios tienen una frecuencia promedio de 14.6 sesiones.
...
Más tiempo de ejecución, menor visibilidad por fraccionar demasiado los datos.
